...
// HunkMod.Modules.Components.MagnumBehavior
...
public class MagnumBehavior : WeaponBehavior
{
    ...
    public override HunkWeaponDef weaponDef => BaseWeapon<Magnum>.instance.weaponDef;
    ...
    public void Start()
    {
        ...
        this.shortBarrel = base.childLocator.FindChild("ShortBarrel").gameObject;
        this.longBarrel = base.childLocator.FindChild("LongBarrel").gameObject;
        this.ammoCache = base.childLocator.FindChild("AmmoCache").gameObject;
        this.laserSight.SetActive(value: false);
        this.laser.SetActive(value: false);
        this.shortBarrel.SetActive(value: true);
        this.longBarrel.SetActive(value: false);
        this.ammoCache.SetActive(value: false);
    }
    ...
    {
        if (base.hunk.characterBody.inventory.GetItemCount(Magnum.dotSight) > 0 && base.hunk.weaponDef == BaseWeapon<Magnum>.instance.weaponDef && base.hunk.isAiming && (bool)this.pointer && this.laser.activeSelf)
        {
            this.UpdatePointer();
        }
    }
    ...
    {
        base.RunFixedUpdate();
        base.TryEnableAttachment(Magnum.ammoCache, this.ammoCache);
        if ((bool)base.hunk)
        {
            bool flag = false;
            if (base.hunk.weaponDef == BaseWeapon<Magnum>.instance.weaponDef)
            {
                flag = true;
            }
            if ((bool)base.hunk.characterBody)
            {
                ...
                {
                    if (base.hunk.characterBody.inventory.GetItemCount(Magnum.dotSight) > 0)
                    {
                    ...
                    if (base.hunk.characterBody.inventory.GetItemCount(Magnum.longBarrel) > 0)
                    {
                        this.shortBarrel.SetActive(value: false);
                        this.longBarrel.SetActive(value: true);
                    }
                    else
                    {
                        this.shortBarrel.SetActive(value: true);
